...
function ChatbotWidget({
  ...
  function sendQuery(text: string) {
    ...
    setTimeout(() => {
      const botResponse = handleChatbotQuery(text, lang);
      const botMsg = { id: (Date.now() + 1).toString(), sender: "bot" as const, text: botResponse.response, intent: botResponse.intent };
      setMessages((prev) => [...prev, botMsg]);

      if (botResponse.autoFiledComplaint && botResponse.generatedReport && onReportGenerated) {
        onReportGenerated(botResponse.generatedReport);
      }
    }, 400);
  }
