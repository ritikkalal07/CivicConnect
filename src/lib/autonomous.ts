...
export function handleChatbotQuery(
  ...
  if (msgLower.includes("pothole") || msgLower.includes("garbage") || msgLower.includes("leak") || msgLower.includes("broken")) {
    return {
      response:
        "I've registered your civic complaint! Detector worker has classified it (Pothole/Garbage) and routed it to Ward 47 Officer Er. R. Sharma (Phone: +91-98765-43210). Reference ID: #CVC-AUTO-" +
